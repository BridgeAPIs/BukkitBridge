...
public class JsonModMessage {
	...
	public static JsonModMessage build(CommandSender sender, String message) {
		if (sender instanceof Player) {
			String prefix = BukkitBridge.get().getPermissionsManager().getPrefix(BukkitBridge.get().getPermissionsManager().getApi().getUser(((Player) sender).getUniqueId()));
	...
	public void send() {
		BukkitBridge.get().getPubSub().send("moderationchan", new Gson().toJson(this));
	}
}
